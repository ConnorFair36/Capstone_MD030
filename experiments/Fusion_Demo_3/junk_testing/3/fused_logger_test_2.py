...
# Camera
FPS = 30
DURATION_SEC = 20  # set to None to run until 'q'
OUT_RGB_DIR = "rgb"
OUT_DEPTH_DIR = "depth"
...
# Raspberry Pi / Linux examples (uncomment and edit):
# CFG_PORT = "/dev/ttyACM0"
# DATA_PORT = "/dev/ttyACM1"
# (or /dev/ttyUSB0 /dev/ttyUSB1 depending on your USB-UART adapters)
...
class RadarLoggerThread:
    ...
    Background radar logger:
      ...
      - Pushes frame events to a queue so the camera loop can "sync" (count radar frames between camera frames)

    NOTE: This does not *perfectly* time-align sensors, but produces a good, ML-friendly record.
    """
            ...
            try:
                while not self._stop_evt.is_set():
                    ...
                    while True:
                        ...
                        # Push event to queue for sync: (radar_host_timestamp_ms, radar_frame_num)
                        try:
                            self.frame_event_q.put_nowait((t_ms, frame_num))
                        except queue.Full:
                            # If queue is full, drop sync events (radar_frames.csv is still the source of truth)
                            pass
...
def main():
    ...
    # Let RealSense choose default resolution for these formats + FPS
    ...
    # Intrinsics + extrinsics (WHAT YOUR GRAD STUDENT WANTS)
    ...
    calib = {
        ...
        "notes": (
            "Depth PNG stores uint16 depth units. Multiply by depth_scale_m_per_unit to get meters. "
            "Extrinsics are depth->color."
        )
    }
    (base_dir / CALIB_JSON).write_text(json.dumps(calib, indent=2))
    ...
    # ML-friendly radar state
    radar_ready = 0
    radar_first_seen_ms = None
    ...
    with open(cam_csv_path, "a", newline="") as fcam, open(sync_csv_path, "a", newline="") as fsync:
        ...
        try:
            while True:
                ...
                # ---- Sync logging: drain radar events (ML-friendly fields) ----
                radar_count = 0
                ...
                if RADAR_ENABLE:
                    while True:
                        ...
                        if radar_ready == 0:
                            radar_ready = 1
                            radar_first_seen_ms = rt_ms

                sync_writer.writerow([
                    frame_id,
                    cam_t_ms,
                    radar_ready,
                    radar_first_seen_ms if radar_first_seen_ms is not None else "NaN",
                    radar_count,
                    last_radar_frame if last_radar_frame is not None else "NaN",
                    last_radar_t_ms if last_radar_t_ms is not None else "NaN",
                ])
                fsync.flush()
