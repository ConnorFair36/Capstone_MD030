...
def main() -> None:
    ...
    depth_pub = roslibpy.Topic(client, "/camera_depth_b64", "std_msgs/String")
    ...
    try:
        while True:
            ...
            depth_heatmap_bgr = data.get("depth_heatmap_bgr")

            detections_json = json.dumps(detections)
            frame_b64 = camera_module.encode_frame_to_b64(frame) if frame is not None else ""
            depth_b64 = camera_module.encode_frame_to_b64(depth_heatmap_bgr) if depth_heatmap_bgr is not None else ""
            ...
            if (not detected) and ((now - last_valid_t) < hold_secs):
                ...
                if depth_b64 == "":
                    depth_b64 = last_valid["depth_b64"]
            ...
            if depth_b64:
                depth_pub.publish(roslibpy.Message({"data": depth_b64}))
