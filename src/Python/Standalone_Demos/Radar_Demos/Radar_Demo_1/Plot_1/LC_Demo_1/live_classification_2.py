...
# ----- Part D: Decision layer parameters -----
MOTION_WINDOW = 8      # how many frames to use for velocity estimation
VEL_THRESH = 0.05      # threshold on |velocity| (m per frame) for stationary vs moving
...
def live_view():
    ...
    # Deque for motion estimation window: store (frame_num, cy)
        ...
        while True:
            ...
            while True:
                ...
                if points_xy:
                    ...
                    # ----- Zone classification (based on distance from radar) -----
                    if cy < 2.0:
                        zone = "NEAR"
                    elif cy < 4.0:
                        zone = "MID"
                    ...
                    # ----- Motion classification using trend over last N frames -----
                    # Add current point to motion window
                    motion_window.append((frame_num, cy))

                    motion = "UNKNOWN"
                    if len(motion_window) >= 3:
                        # Extract arrays
                        frames_arr = np.array([f for (f, _) in motion_window], dtype=float)
                        cy_arr = np.array([y for (_, y) in motion_window], dtype=float)

                        # Normalize time axis to start at 0 for numerical stability
                        t = frames_arr - frames_arr[0]

                        # Fit line cy ~ slope * t + intercept
                        slope, intercept = np.polyfit(t, cy_arr, 1)
                        vel_est = slope  # meters per frame (approx)

                        # Interpret velocity
                        if abs(vel_est) < VEL_THRESH:
                            motion = "STATIONARY"
                        elif vel_est > 0:
                            motion = "MOVING AWAY"
                        else:
                    ...
                    print(
                        f"Frame {frame_num}: zone={zone}, motion={motion}, "
                        f"dist={dist:.2f} m, vel_est={vel_est:.3f} m/frame"
                    )
                    ...
                    # keep existing trail (optional: uncomment to clear)
                    # trail.clear()
                    # trail_line.set_data([], [])
                ...
                if points_xy:
                    ax.set_title(
                        f"Frame {frame_num} | {zone} | {motion} | dist={dist:.2f} m"
                    )
                else:
